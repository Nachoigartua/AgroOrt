...
Frontend Angular
: Interfaz de usuario integrada como iframe
API Gateway
: Capa de comunicación con sistema principal
ML Engine
: Motor de procesamiento de modelos
...
2.1 Frontend
Framework
: Angular 15+ (compatible con componentes empresa)
UI Components
: Librería de componentes proporcionada por la empresa
State Management
: NgRx para manejo de estado complejo
Charting
: Chart.js o D3.js para visualizaciones
Maps
: Leaflet para visualización geográfica
2.2 Backend
Runtime
: Python 3.9+
Framework
...
get_recomendaciones_siembra(request:
...
get_recomendaciones_variedades(request:
...
get_predicciones_clima(request:
...
get_recomendaciones_fertilizacion(request:
...
get_predicciones_rendimiento(request:
...
RecomendacionResponse(BaseModel):
...
validate_token_with_main_system(token)
...
process_batch_predictions(lote_ids:
...
main_system_client.validate_token(token.credentials)
...
protected_endpoint(user=Depends(verify_token)):
...
test_predict_fecha_siembra_valid_input(self):
...
test_predict_fecha_siembra_invalid_input(self):
...
test_siembra_endpoint(client):
    
response
 
=
 
client.post("/api/v1/recomendaciones/siembra",
...
test_full_prediction_workflow(self):
...
test_main_system_integration(self):
...
test_prediction_response_time(self):
...
test_concurrent_predictions(self):
...
idx_predicciones_lote_fecha
 
ON
 
predicciones(lote_id,
...
idx_clima_historico_coords_fecha
 
ON
 
clima_historico(latitud,
...
idx_caracteristicas_suelo_lote
 
ON
 
caracteristicas_suelo(lote_id);
--
...
self._get_relevant_params(prediction_type,
 
params)
        
return
 
f"{prediction_type}:{hash(frozenset(relevant_params.items()))}"

